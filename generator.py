...
class ConvBlock(nn.Module):
    def __init__(self, in_channels, out_channels, use_act = True, **kwargs):
        super().__init__()
        self.conv = nn.Conv2d(in_channels, out_channels, **kwargs, bias = False, padding_mode = 'reflect')
...
class Block(nn.Module):
    def __init__(self, in_channels, out_channels, stride = 2, down=True, act="relu"):
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(in_channels, out_channels, kernel_size = 3, stride = stride, padding = 1, bias = False, padding_mode = 'reflect'),
...
class Generator(nn.Module):
    ...
    def forward(self, x):
        ...
        return self.finalconv(torch.cat([u4, d1], dim=1))
